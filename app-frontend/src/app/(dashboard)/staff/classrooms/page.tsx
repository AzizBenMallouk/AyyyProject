...
export default function StaffClassroomListPage() {
    ...
    useEffect(() => {
        const fetchOptions = async () => {
            try {
                const [c, p, g, t, pr] = await Promise.all([
                    getAllCampuses(),
                    getAllPromotions(),
                    getAllGrades(),
    ...
    useEffect(() => {
        fetchClassrooms();
    }, [user?.id, campusId, promotionId, gradeId, programId, trainerId, active, page]);

    // Reset page to 0 when filters change
    useEffect(() => {
        setPage(0);
    }, [campusId, promotionId, gradeId, programId, trainerId, active]);
            ...
            <div className="p-8 space-y-8 min-h-screen text-slate-200">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 xl:grid-cols-7 gap-3 bg-black/40 p-4 rounded-xl border border-white/5 backdrop-blur-xl">
                    ...
                    <select
                        className="w-full p-2 bg-white/5 border border-white/10 rounded-lg text-xs text-white bg-[#0a0a0f]"
                        value={programId || ""}
                        onChange={(e) => setProgramId(e.target.value ? parseInt(e.target.value) : undefined)}
                    >
