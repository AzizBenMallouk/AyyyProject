...
export default function StudentDataTable({
    ...
    const fetchLearners = async () => {
        ...
        try {
            const data = await getFilteredLearners({
                ...
                classroomId: classroomId, // Pass classroomId here
    ...
    useEffect(() => {
        setPage(0);
    }, [filters.gradeId, filters.promotionId, filters.campusId, filters.status, filters.query]);
    ...
    return (
        <div className="space-y-4">
            <div className="flex flex-col md:flex-row gap-4">
                ...
                <div className="flex gap-2">
                    <Button
                        ...
                        className={`bg-white/5 border-white/10 ${showFilters ? 'bg-primary/20 border-primary' : ''}`}
                    >
            ...
            {showFilters && (
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 p-4 rounded-xl bg-white/5 border border-white/10 animate-in fade-in slide-in-from-top-2 duration-300">
                    <div className="space-y-1.5">
                        <label className="text-[10px] uppercase font-bold text-muted-foreground">Grade</label>
                        <select
                            value={filters.gradeId || ""}
                            onChange={(e) => setFilters({ ...filters, gradeId: e.target.value ? parseInt(e.target.value) : undefined })}
                            className="w-full bg-[#0a0a0f] border border-white/10 rounded-md p-2 text-sm text-white focus:outline-none focus:ring-1 focus:ring-primary"
                        >
                    ...
                    <div className="space-y-1.5">
                        <label className="text-[10px] uppercase font-bold text-muted-foreground">Promotion</label>
                        <select
                            value={filters.promotionId || ""}
                            onChange={(e) => setFilters({ ...filters, promotionId: e.target.value ? parseInt(e.target.value) : undefined })}
                            className="w-full bg-[#0a0a0f] border border-white/10 rounded-md p-2 text-sm text-white focus:outline-none focus:ring-1 focus:ring-primary"
                        >
                    ...
                    <div className="space-y-1.5">
                        ...
                        <select
                            ...
                            onChange={(e) => setFilters({ ...filters, campusId: e.target.value ? parseInt(e.target.value) : undefined })}
                            className="w-full bg-[#0a0a0f] border border-white/10 rounded-md p-2 text-sm text-white focus:outline-none focus:ring-1 focus:ring-primary"
                        >
                    ...
                    <div className="space-y-1.5">
                        ...
                        >
                            ...
                            {["ACTIVE", "INACTIVE", "SUSPENDED", "GRADUATED"].map(s => (
